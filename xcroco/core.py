...
class Xroms:
    ...
    - generalized calc_optic_romspisces for output that does not contain the variable AKt'
    '''
            ...
            try:
                ...
                if vtrans.size is not 0:
                    if vtrans.size == 1:
                        ...
                        # self.s_coord = int(vtrans.isel(time=0))   <-- fails without timne dimension
    ...
    def _zlevs(self, var, **kwargs):
        ...
        if vtransform == 2:
            ...
            for k in np.arange(0, N):
                z0 = cff[k] + cff1[k] * h
                z.loc[{str(vertical_dim):var[vertical_dim][k]}] = zeta * (1. + z0 * h2inv) + z0 * h / (h2)
            ...
            for k in np.arange(0, N):
                z0 = cff[k] + cff1[k] * h
                #z.loc[{str(vertical_dim):var[vertical_dim][k]}] = zeta * (1. + z0 * hinv) + z0  # <-- fails with dask array
                level_slice = zeta * (1. + z0 * hinv) + z0
                z = z.where(var[vertical_dim] != var[vertical_dim][k], other=level_slice)
